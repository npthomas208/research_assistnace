...
```{r}
#Most common words
commonwords <- gsub("[[:punct:][:blank:]]+", " ", survey_tbl$`Death Thoughts1`) %>% tokenize_word() %>% unlist() %>% data_frame(word = .) %>% filter(word != " ") %>% count(word, sort=T) 

#Most common bi- trigrams
...
survey_tbl <- survey_tbl %>%
  mutate(cliche = str_detect(`Death Thoughts1`,commonwords$word))
```
